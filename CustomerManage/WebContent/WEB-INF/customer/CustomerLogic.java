...
public class CustomerLogic {
    ...
    public String add(CustomerBean customer) {
        ...
        CustomerDao customerDao = new CustomerDao();
//        boolean isAdded = customerDao.add(customer) != null;

//        System.out.println(errMessage); //エラーメッセージの中身チェック
...
//        }
    ...
    public String update(CustomerBean customer) {
        ...
        CustomerDao customerDao = new CustomerDao();
        //boolean isUpdated = customerDao.update(customer) == null;
    	String errMessage = customerDao.update(customer);

//        if (isUpdated) {
//            return null; // 成功時はエラーメッセージなし
//        } else {
//            String errMessage = MESSAGE_CAN_NOT_UPDATE;
//            return errMessage;
//        }
        if (errMessage != null) {
    ...
    public String delete(CustomerBean customer) {
        ...
        CustomerDao customerDao = new CustomerDao();
        boolean isDeleted = customerDao.delete(customer.getId()) == null;

        if (isDeleted) {
            return null; // 成功時はエラーメッセージなし
        } else {
            String errMessage = MESSAGE_CAN_NOT_DELETE;
            return errMessage;
        }
    }
