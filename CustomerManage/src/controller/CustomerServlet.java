...
public class CustomerServlet extends BaseServlet {
    ...
    private void procUpdate(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        //customerLogic.setCustomerBeanFromRequestToSession(request);
         CustomerBean customer=(CustomerBean)session.getAttribute("customer");    //セッションから顧客情報を取得

         String errMessage = null;
         errMessage = customerLogic.update(customer);//DB更新処理

         session.removeAttribute("customer");
         if(errMessage==null) {              //画面遷移
        getServletContext().getRequestDispatcher("/WEB-INF/customer/update_success.jsp").forward(request, response);
         }else {
            //errMessage+=MessageConstants.MESSAGE_CAN_NOT_UPDATE;
    ...
    private void procAdd(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        CustomerLogic customerLogic=new CustomerLogic();
         ...
         CustomerBean customer=(CustomerBean)session.getAttribute("customer");  //ここまで、セッションから顧客情報を取得
//         if(customer==null) {errMessage+=MessageConstants.MESSAGE_CAN_NOT_ADD;
//             request.setAttribute("errMessage",errMessage);
//             getServletContext().getRequestDispatcher("/WEB-INF/customer/add_fail.jsp");
//             return;
//         }
         errMessage=customerLogic.add(customer);         //DB追加処理後、画面遷移
         session.removeAttribute("customer");
      if(errMessage==null) {
       getServletContext().getRequestDispatcher("/WEB-INF/customer/add_success.jsp").forward(request, response);
       }else {
        //errMessage+=MessageConstants.MESSAGE_CAN_NOT_ADD;
        //request.setAttribute("errMessage", errMessage);
       session.setAttribute("errMessage", errMessage);
...
//        int intId = Integer.parseInt(request.getParameter(id));
    ...
    private void procDelete(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        CustomerLogic customerLogic= new CustomerLogic();
        String errMessage=null;
//        if(customer==null) {errMessage+=MessageConstants.MESSAGE_NO_EXIST_DATA_TO_DELETE;
//            request.setAttribute("errMessage",errMessage);
//            getServletContext().getRequestDispatcher("/WEB-INF/customer/delete_fail.jsp").forward(request, response);
//            return;
//        }
...
//           session.removeAttribute("customer");
           if(errMessage==null) {
            ...
            //errMessage+=MessageConstants.MESSAGE_CAN_NOT_DELETE;
            //request.setAttribute("errMessage", errMessage);
               session.setAttribute("errMessage", errMessage);
               getServletContext().getRequestDispatcher("/WEB-INF/customer/delete_fail.jsp").forward(request, response);
           }
