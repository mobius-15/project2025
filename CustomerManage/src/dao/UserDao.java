...
public class UserDao extends BaseDao {
    ...
    public String updateUser(String newPwd, int id, String oldPwd) {
        ...
        PreparedStatement pstmt = null;
        String strSql = "UPDATE login_user " +
            "SET pwd = TO_BASE64(AES_ENCRYPT(?, RPAD(?, 32, CHAR(0)))) " +
            "WHERE id = ? AND pwd = TO_BASE64(AES_ENCRYPT(?, RPAD(?, 32, CHAR(0))))";

        try {
            open();
            pstmt = conn.prepareStatement(strSql);
            pstmt.setString(1, newPwd);         // 新しい平文パスワード
            pstmt.setString(2, SECRET);         // 暗号キー
            pstmt.setInt(3, id);
            pstmt.setString(4, oldPwd);         // 現在の平文パスワード
            pstmt.setString(5, SECRET);         // 暗号キー（再度）
