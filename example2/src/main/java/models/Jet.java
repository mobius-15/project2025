...
public class Jet {
	...
	private int eWeight,intTank;
	int speed,hour;
	private double wingArea;
	 static int fullWeight;
...
public Jet(String type,String mission){
		Jet.type=type;
...
public void printData(){
   System.out.println("機種："+ Jet.type);
   System.out.println("機数："+ Jet.elements);
   System.out.println("飛行距離："+ Jet.distance+"nm");
   System.out.println("燃料："+Jet.fuel+" lb");
   System.out.println("総重量："+Jet.fullWeight+" lb");
}
public void information(Airbase base) {
	...
	System.out.println(base.name+"所属機:"+Jet.type+"\n"+"機数は"+(Jet.elements)*2+"機");
	
}

public int elements(int sorties) {
	int planes=sorties*Jet.elements;
	return planes;
}
public int[]fly(int distance) {
 ...
 int[]wayPoints= new int[distance];
	...
	if(wayPoints[w]>=30||wayPoints[w]<=1) {System.out.println("無効な数値");continue;}		
				
		Jet.distance+=wayPoints[w];
...
public int fullFuel(int tankN) {
	tankN*=3000;
	 if(Jet.type.equals("F-35B")){setIntTank(13100);
	}else if(Jet.type.equals("F/A-18E")) {setIntTank(14480);}
	 if(Jet.type.equals("F-35B")||Jet.type.equals("F-35C")) { tankN=0;}
	Jet.fuel=getIntTank()+tankN;
	return Jet.fuel;
...
public Airbase getBase() {
	return base;
}
public void setBase(Airbase base) {
	this.base = base;
}
public void setStallSpeed(int stallSpeed) {
